 ...
 * RUNNING -> OBSERVING -> NEEDS YOU -> EVIDENCE -> APPROVE -> RESUMING -> RESOLVED
 */
export default function LiveSystemControlLoop() {
  ...
  const stages = [
    {
      ...
      num: '01',
      label: 'RUNNING',
      title: 'Frontend, backend, and agents run in parallel.',
      subtitle: 'Microservices, databases, and autonomous agents execute in supervised PTY sessions with zero manual log staring.',
      tag: '01 // PARALLEL EXECUTION',
    ...
    },
    {
      ...
      num: '02',
      label: 'OBSERVING',
      title: 'OUTARCH continuously watches process streams.',
      subtitle: 'Stream parsers extract structured facts in real-time: listening ports, URL endpoints, and passing test counts.',
    ...
    },
    {
      ...
      num: '03',
      label: 'NEEDS YOU',
      title: 'Something goes wrong. OUTARCH flags the blocker.',
      subtitle: 'An authentication test assertion fails. Instead of burying it in hundreds of scrolling lines, OUTARCH routes one item to the Needs You queue.',
    ...
    },
    {
      ...
      subtitle: 'CrashLens isolates the failing stack trace, exact file line, and context diff so you can inspect the root cause immediately.',
    ...
    },
    {
      ...
      num: '05',
      label: 'APPROVE',
      title: 'Operator approves the fix with 1 click.',
      subtitle: 'Mission AI synthesizes the JWT signing key fix. You review the proposed mutation and click Approve.',
      tag: '05 // OPERATOR APPROVAL',
      badgeColor: 'badge-ai',
    },
    {
      id: 'resolved',
      num: '06',
      label: 'RESOLVED',
      title: 'Agent resumes. All systems return to green.',
      subtitle: 'The patch is applied, tests pass immediately, and all monitored services resume full health.',
  ...
  const handleInspectClick = () => {
    setAutoplay(false);
    setActiveStage(3); // Go to EVIDENCE
  };

  const handleApproveClick = () => {
    setAutoplay(false);
    setActiveStage(4); // Go to APPROVE
    setTimeout(() => {
      setActiveStage(5); // Transition to RESOLVED
    }, 1200);
  };

  return (
    <section id="control-loop" className="py-24 px-6 max-w-7xl mx-auto select-none border-t border-white/[0.08]">
      
      {/* Editorial Header */}
      <div className="max-w-3xl mb-12">
        <span className="font-mono text-xs text-sky-400 font-semibold uppercase tracking-wider block mb-3">
          // SIGNATURE OUTARCH SYSTEM ANIMATION
        </span>
        <h2 className="font-display text-3xl sm:text-5xl font-black text-titanium uppercase leading-[1.05]">
          Watch OUTARCH in action.
        </h2>
        <p className="font-sans text-slate-400 text-base sm:text-lg mt-4 leading-relaxed">
          The developer should not have to watch everything. OUTARCH watches. You decide.
        </p>
      </div>

      {/* Chapter Progress Scrubber */}
      <div className="w-full flex items-center justify-between gap-1 p-1 rounded-xl bg-[#0A0E18] border border-white/10 mb-8 overflow-x-auto no-scrollbar font-mono text-xs">
        {stages.map((st, idx) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch rounded-2xl p-6 sm:p-8 border border-white/10 bg-[#080B12] shadow-2xl">
        
        {/* Left: Narrative Context & Controls */}
        <div className="lg:col-span-5 flex flex-col justify-between space-y-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-[#0A0E18] border border-white/10 font-mono text-xs text-slate-300 font-semibold mb-4">
        ...
        <div className="lg:col-span-7 rounded-xl bg-[#0A0E18] border border-white/10 p-5 font-mono text-xs space-y-3">
          ...
          <div className="space-y-2">
            ...
            {/* Worker 1: Frontend */}
            <div className="p-3 rounded-lg bg-[#05060A] border border-white/5 flex items-center justify-between hover:border-white/10 transition-colors">
            ...
            <div className={`p-3 rounded-lg border transition-all ${
              activeStage === 2
                ...
                : 'bg-[#05060A] border-white/5 text-slate-300'
            ...
            <div className={`p-3 rounded-lg border transition-all ${
              ...
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-slate-500 text-[10px]">Claude Code (Autonomous Task)</span>
              </div>
              <span className="badge-ai px-2 py-0.5 rounded text-[10px] font-bold">
                {activeStage <= 1 && 'WORKING'}
                {activeStage === 2 && 'NEEDS YOU'}
                {activeStage === 3 && 'ANALYZING EVIDENCE'}
                {activeStage === 4 && 'APPROVING FIX'}
                {activeStage === 5 && 'RESUMING'}
              </span>
            </div>

          </div>

          {/* Dynamic Attention & Evidence Card */}
          {activeStage >= 2 && (
            <div className="mt-4 p-4 rounded-xl bg-[#05060A] border border-white/10 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs font-bold text-orange-400">
                  <AlertTriangle className="w-4 h-4" />
                  <span>NEEDS YOU: Authentication Test Failed</span>
                </div>
                {activeStage === 2 && (
                  <button 
                    onClick={handleInspectClick}
                    className="px-2.5 py-1 rounded bg-orange-500/20 hover:bg-orange-500/30 border border-orange-500/40 text-orange-300 text-[10px] font-bold flex items-center gap-1 transition-all"
                  >
              ...
              {activeStage >= 3 && (
                <div className="p-3 rounded-lg bg-[#0A0E18] border border-red-500/20 text-[11px] text-slate-300 font-mono space-y-1">
                  ...
                  <div className="text-slate-500 text-[10px]">Root cause: Expired test JWT secret fixture in auth pipeline</div>
                </div>
              )}
              ...
              {activeStage >= 4 && (
                <div className="flex items-center justify-between pt-2 border-t border-white/5">
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-lg btn-primary text-xs font-bold font-mono shadow-sm"
                  >
                    1-Click Approve Fix
                  </button>
                </div>
